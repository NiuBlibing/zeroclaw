...
| Package | Why |
|---|---|
| `rust` | Provides `cargo` and `rustc` to build the binary. ZeroClaw's workspace MSRV is Rust 1.97.1; the FreeBSD `rust` port tracks a newer stable, so `pkg install rust` satisfies it. |
